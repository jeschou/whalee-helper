...
public class GoRepo2Generator {

    protected static Notifier notifier = Notifier.getInstance("whgo_helper gorm-repo2-gen");
    ...
    public GoRepo2Generator(Project project, VirtualFile selectedFile, DbConfig dbConfig, String fileName, String database, String tableName, String structName, List<TableRowData> fields) {
    ...
    private void generate0() throws IOException {
        ...
        try {
            templateRender.loadTemplate(GoRepo2Generator.class.getResourceAsStream("repo_template.txt"));
        } catch (IOException e) {
            e.printStackTrace();
        ...
        for (TableRowData trd : fields) {
            repoFieldsSb.append("\t").append(String.format("%-" + maxFieldWidth + "s repo2.FieldInterface", trd.fieldName)).append("\n");
        }
...
//            RefreshQueue.getInstance().refresh(true, true, null, targetDir);
    ...
    private List<String> collectImport() throws IOException {
        ...
        sets.add("sync");
        sets.add("gitlab.meetwhale.com/whale/whale-framework/frameworks/repo2");
...
//        }
        for (TableRowData trd : fields) {
            ...
            if (goType.contains("datatypes.")) {
                sets.add("gorm.io/datatypes");
            }
            if ("is_delete".equals(trd.name) || "is_deleted".equals(trd.name)) {
                sets.add("context");
                sets.add("time");
